"""
QueryTube - Channel-Specific YouTube Search
Search videos within a specific YouTube channel!
"""
...
# Page configuration
st.set_page_config(
    page_title="QueryTube - Channel Search",
    page_icon="🎥",
    layout="wide"
)
...
# Search section
st.markdown("### 🔍 Search Videos in a Channel")

# Channel input
channel_input = st.text_input(
    "Enter YouTube Channel Name or Handle",
    placeholder="e.g., TED, @mkbhd, Khan Academy",
    help="Enter channel name (e.g., TED) or handle (e.g., @channelname)"
)
...
with col1:
    query = st.text_input(
        ...
        placeholder="Type your search query... (e.g., 'python', 'climate change', 'tutorial')",
...
with col1:
    if st.button("🎓 TED", use_container_width=True):
        st.session_state.clicked_channel = "TED"
        st.rerun()

with col2:
    if st.button("📺 Khan Academy", use_container_width=True):
        st.session_state.clicked_channel = "Khan Academy"
        st.rerun()

with col3:
    if st.button("🎬 Crash Course", use_container_width=True):
        st.session_state.clicked_channel = "Crash Course"
        st.rerun()

with col4:
    if st.button("🔬 Kurzgesagt", use_container_width=True):
        st.session_state.clicked_channel = "Kurzgesagt"
        st.rerun()
...
with col5:
    if st.button("💻 freeCodeCamp", use_container_width=True):
        st.session_state.clicked_channel = "freeCodeCamp"
        st.rerun()

with col6:
    if st.button("🌍 Nat Geo", use_container_width=True):
        st.session_state.clicked_channel = "National Geographic"
        st.rerun()

with col7:
    if st.button("📚 MIT", use_container_width=True):
        st.session_state.clicked_channel = "MIT OpenCourseWare"
        st.rerun()

with col8:
    if st.button("🎓 Stanford", use_container_width=True):
        st.session_state.clicked_channel = "Stanford Online"
        st.rerun()

# Handle channel button clicks
if 'clicked_channel' in st.session_state:
    channel_input = st.session_state.clicked_channel
    del st.session_state.clicked_channel
    st.rerun()

# Perform search
if search_button:
    if not channel_input:
        st.error("❌ Please enter a YouTube channel name first!")
    elif not query:
        st.error("❌ Please enter a search query!")
    else:
        # Get channel ID first
        with st.spinner(f"🔍 Finding channel '{channel_input}'..."):
            channel_id = searcher.get_channel_id(channel_input)
        
        if not channel_id:
            st.error(f"❌ Could not find channel: {channel_input}")
        else:
            with st.spinner(f"🔍 Searching videos in '{channel_input}' for '{query}'..."):
                videos = searcher.search_videos(query, max_results=20, channel_id=channel_id)
            ...
            if videos:
                ...
                if videos:
                    st.success(f"✅ Found {len(videos)} video{'s' if len(videos) != 1 else ''} in {channel_input}!")
                else:
...
# Show how it works section (always visible)
st.markdown("---")
st.markdown("""
### ✨ How it works:
1. **Enter a channel name** (e.g., TED, Khan Academy) or click a popular channel button
2. **Type your search query** to find videos within that channel
3. **Click Search** - we'll search within the specific channel
4. **Get top 5 videos** matching your query from that channel
5. **Watch directly** in the app or click to open in YouTube
6. **View transcripts** if available (click the dropdown)

### 🎯 Channel-Specific Search:
- ✅ Search **within specific YouTube channels**
- ✅ Get **instant results** from your favorite creators
...
<div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
    ...
    <p style="color: white !important; margin: 0; font-size: 14px;">
        ⚡ Powered by YouTube Data API v3 • 📺 Channel-Specific Search • 🚀 Instant Results
    </p>
</div>
""", unsafe_allow_html=True)
